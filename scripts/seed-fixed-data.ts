...
async function seedRealisticData() {
  ...
  // 1. 清理现有的测试用户（保留管理员）
  await db.run(`DELETE FROM users WHERE email != 'admin@i8relay.com' AND role != 'super_admin'`);

  // 2. 首先创建套餐数据，因为用户需要引用套餐ID
  console.log('📦 创建套餐数据...');
  ...
  for (const user of users) {
    try {
      await db.run(`
        INSERT INTO users (id, username, email, password_hash, salt, role, current_plan_id, balance, phone, company, total_requests, total_tokens, total_cost, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
      `, [
        user.id,
        user.username,
  ...
  console.log('\n📝 测试用户账户信息:');
  console.log('邮箱: zhangming@example.com | 密码: 123456 (Claude Code 标准版)');
  console.log('邮箱: lixiaohua@tech.com | 密码: 123456 (Claude Code 专业版)');
  console.log('邮箱: chen@architect.com | 密码: 123456 (CodeX 企业版)');
  console.log('邮箱: admin@i8relay.com | 密码: admin123 (管理员)');
}
